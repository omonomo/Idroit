...
# Custom font generator for Idroit
#
...
address_store_zero=$((address_store_braille + 256)) # 保管したドット無し0・スラッシュ0アドレス
...
address_vert_start_liga="1114243" # 合成後のvert置換の先頭アドレス (リガチャあり)
lookupIndex_liga_end_liga="96" # リガチャ用caltの最終lookupナンバー (リガチャあり)
...
address_init() {
    ...
    num_cv_lookups="34" # cvのルックアップ数 (lookupの数を変えた場合はtable_modificatorの内容も変更すること)
}
# 著作権
copyright="Copyright (c) 2025 omonomo\n\n"
copyright="${copyright}\" + \"[Monoid]\nCopyright (c) 2016, Andreas Larsen\n\n"
...
name_vendor_url="https://github.com/omonomo/Idroit"
name_designer=""
name_designer_url=""
...
# 上付き、下付き用
scale_width_super_sub="74" # 基本から作成する上付き・下付き文字のX座標拡大率
scale_height_super_sub="74" # 基本から作成する上付き・下付き文字のY座標拡大率
weight_super_sub="14" # ウェイト調整量
move_y_super="490" # 基本から作成した上付き文字のY座標移動量 (すでにある上付き文字のベースラインのY座標)

center_height_super_sub="364" # 上付き、下付き文字のY座標拡大中心
scale_super_sub2="100" # 上付き、下付き文字の拡大率
move_y_super2="0" # 上付き文字のY座標移動量
move_y_sub="-262" # 下付き文字のY座標移動量
...
# 縦書き全角記号移動量
move_x_vert_colon="79" # ：；
move_x_vert_bar="20" # ｜
move_x_vert_solidus="-9" # ／＼
move_x_vert_math="50" # ＝－＜＞
move_y_vert_bbar="-8" # ￤
...
calt_init() {
    ...
    move_y_calt_colon=$((move_y_math + 43)) # : のY座標移動量 (= 基準)
    ...
    move_y_calt_bar=$((move_y_math - 43)) # | のY座標移動量 (= 基準)
    ...
    move_y_calt_math=$((- move_y_math + 100)) # +-= のY座標移動量 (括弧基準)
    ...
    move_y_calt_colon2="145" # : のY座標移動量 (括弧基準)
    ...
    move_y_calt_bar2="60" # | のY座標移動量 (括弧基準)
...
width_latin="682" # Latin フォントの em 値を1024に変換したときの文字幅
...
# Set filenames
origin_latin_regular="Monoid-Retina.ttf"
origin_latin_bold="Monoid-Bold.ttf"
...
# Clear kerns, position, substitutions
    ...
    if (input_list[i] == "${input_latin_regular}") # Bold に存在しないリガチャを削除
        lookupSubs = GetLookupSubtables(lookups[14])
        RemoveLookupSubtable(lookupSubs[136])
        Print("Remove GSUB_" + lookups[14] + ": " + lookupSubs[136])
    endif
#    if (input_list[i] == "${input_latin_bold}") # Retina より少ないグリフと Lookupを埋めるダミー
#        lookups = GetLookups("GSUB"); numlookups = SizeOf(lookups)
#        lookupName = "単純置換 lookup 110"
...
#        Select(0u002a) # *
#        AddPosSub(lookupSub0, glyphName)
#    endif
    while (j < numlookups)
        if ("${liga_flag}" == "false" || j < 14 || 109 < j)
...
# Retina と Regular、Bold での位置ズレ補正
...
# D (ss 用、クロスバーを付加することで少しくどい感じに)
    ...
    if (input_list[i] == "${input_latin_regular}")
        Scale(65, 100); Copy()
        Select(0u0044) # D
        PasteWithOffset(-213, -339)
    else
        Scale(80, 100); Copy()
        Select(0u0044) # D
        PasteWithOffset(-208, -393)
    endif
    SetWidth(${width_latin})
    RemoveOverlap()
    Select(65575);  Clear() # Temporary glyph

# J (リーゼントを少しカットして少し左に移動)
    ...
    PasteWithOffset(0, -500)
    PasteWithOffset(0, 1115)
    RemoveOverlap()
    Copy()
    Select(0u004a) # J
    SelectMore(0u0134) # Ĵ
 ...
 #    SelectMore(0ua7b2) # Ʝ
    ...
    SetWidth(${width_latin})
    # キリル文字
    Select(0u004a); Copy() # J
    Select(0u0408); Paste() # Ј
    SetWidth(${width_latin})
    Select(65575); Clear() # Temporary glyph
...
# V (ss用、左上にセリフを追加してYやレと区別しやすく)
    ...
    if (input_list[i] == "${input_latin_regular}")
        Scale(65, 100); Copy()
        Select(0u0056); # V
        PasteWithOffset(-250, 88) # V
    else
        Scale(80, 100); Copy()
        Select(0u0056); # V
        PasteWithOffset(-205, 0) # V
    endif
...
# Z (ss用、クロスバーを付加してゼェーットな感じに)
    ...
    Select(0u00af); Copy()  # macron
    Select(65575);  Paste() # Temporary glyph
    Scale(100, 100); Rotate(-2)
    Copy()
    Select(0u005a) # Z
    if (input_list[i] == "${input_latin_regular}")
        PasteWithOffset(-7, -341)
    else
        PasteWithOffset(-7, -389)
    endif
    SetWidth(${width_latin})
    RemoveOverlap()
    Select(65575);  Clear() # Temporary glyph
...
# g (long のグリフに置換)
    Select("g.long"); Copy();  Select("g"); Paste(); SetWidth(${width_latin})
    Select(0u2588); Copy() # Full block
    Select(0u011d); PasteWithOffset(0, 870); OverlapIntersect() # ĝ
    Select(0u011f); PasteWithOffset(0, 870); OverlapIntersect() # ğ
    Select(0u0121); PasteWithOffset(0, 870); OverlapIntersect() # ġ
    Select(0u0123); PasteWithOffset(0, 870); OverlapIntersect() # ģ
 #    Select(0u01e5) # ǥ
 #    Select(0u01e7) # ǧ
 #    Select(0u01f5) # ǵ
 #    Select(0u0260) # ɠ
 ...
 #    Select(0ua7a1) # ꞡ
    ...
    Select(0u011d); PasteInto(); SetWidth(${width_latin})
    Select(0u011f); PasteInto(); SetWidth(${width_latin})
    Select(0u0121); PasteInto(); SetWidth(${width_latin})
    Select(0u0123); PasteInto(); SetWidth(${width_latin})
...
# j (long のグリフに置換)
    Select("j.long"); Copy();  Select("j"); Paste(); SetWidth(${width_latin})

# j (short グリフに統一)
 #    Select(0u2588); Copy() # Full block
 #    Select(0u0135); PasteWithOffset(100, 880) # ĵ
 #    OverlapIntersect()
 #
 #    Select(0u2588); Copy() # Full block
 #    Select(65575);  Paste() # Temporary glyph
 ...
 #    Select(65575);  PasteInto() # Temporary glyph
 #    OverlapIntersect()
 #    Copy()
 #    Select(0u0135); PasteInto() # ĵ
 #    SetWidth(${width_latin})
 #    Select(65575); Clear() # Temporary glyph
 #
 #    Select(0u006a); Copy() # j
 ...
 # #    SelectMore(0u03f3) # ϳ
 #    # キリル文字
 #    Select(0u0458); Paste() # ј
 #    SetWidth(${width_latin})

# l (少し左に移動) ※ Retina では不要
 #    Select(0u006c) # l
 #    SelectMore(0u013a) # ĺ
 #    SelectMore(0u013c) # ļ
 #    SelectMore(0u013e) # ľ
...
# y (long のグリフに置換)
    Select("y.long"); Copy();  Select("y"); Paste(); SetWidth(${width_latin})
    Select(0u2588); Copy() # Full block
    Select(0u00fd); PasteWithOffset(0, 870); OverlapIntersect() # ý
    Select(0u00ff); PasteWithOffset(0, 870); OverlapIntersect() # ÿ
    Select(0u0177); PasteWithOffset(0, 870); OverlapIntersect() # ŷ
 #    Select(0u01b4) # ƴ
 #    Select(0u0233) # ȳ
 #    Select(0u024f) # ɏ
 #    Select(0u1e8f) # ẏ
 ...
 #    Select(0uab5a) # ꭚ
    ...
    Select(0u00fd); PasteInto(); SetWidth(${width_latin})
    Select(0u00ff); PasteInto(); SetWidth(${width_latin})
    Select(0u0177); PasteInto(); SetWidth(${width_latin})
...
# ☠ (縮小して左に移動)
    Select(0u2620) # ☠
    Move(-38, 0)
    Scale(77, ${width_latin} / 2, 0)
    SetWidth(${width_latin})

    Print("Edit zero")
# ドット無し0を保管
    # 通常 (上付き、下付きは後で加工)
    Select("zero.clear"); Copy() # ドット無し0
...
# 0 (dot のグリフに置換)
    Select("zero.dot"); Copy();  Select("zero"); Paste(); SetWidth(${width_latin})
...
# ⁄ (/と区別するため分割)
    ...
    Select(0u2044) # ⁄
    if (input_list[i] == "${input_latin_regular}")
        PasteWithOffset(330, 700); PasteWithOffset(-330, -700)
    else
        PasteWithOffset(285, 720); PasteWithOffset(-285, -720)
    endif
    OverlapIntersect()
    Rotate(-10)
    SetWidth(${width_latin})
...
# 上付き、下付き文字を追加、sups、subs フィーチャを追加
    ...
    orig = [0u025b, 0u025c] # ɛɜ
    sups = [0u1d4b, 0u1d9f] # ᵋᶟ # グリフ作成
    j = 0
    while (j < SizeOf(orig))
        Select(orig[j]); Copy()
        Select(sups[j]); Paste()
 ...
 #            # ɐɑɒɔ ɕɟ ɡɥɨɩ ɪɭɯɰ ɱɲɳɴ ɵɸʂʃ ʉʊʋʌ ʐʑʒʝ ʟɦɹɻ ʁʔʕɣ
 ...
 #            # ᵄᵅᶛᵓ ᶝᶡ ᶢᶣᶤᶥ ᶦᶩᵚᶭ ᶬᶮᶯᶰ ᶱᶲᶳᶴ ᶶᶷᶹᶺ ᶼᶽᶾᶨ ᶫʱʴʵ ʶˀˁˠ # グリフ作成
 #    j = 0
 #    while (j < SizeOf(orig))
 #        Select(orig[j]); Copy()
 #        Select(sups[j]); Paste()
...
# \ (ss13 用のグリフを作る)
    ...
    Scale(120); Copy()
    if (input_list[i] == "${input_latin_regular}")
        Move(-35, 0); PasteWithOffset(35, 0)
    else
        Move(-75, 0); PasteWithOffset(75, 0)
    endif
...
# < (ss18 用のグリフを作る)
    ...
    Select(65575); Paste() # Temporary glyph
    if (input_list[i] == "${input_latin_regular}")
        Scale(170, 128, 384 + ${move_y_math})
    else
        Scale(155, 85, 384 + ${move_y_math})
    endif
    Copy()
    Select(${address_store_ssliga}); Paste() # 保管所
    PasteWithOffset(100, 0); RemoveOverlap(); Copy()
    if (input_list[i] == "${input_latin_regular}")
        Move(0, 21)
        PasteWithOffset(0, -21)
    else
        Move(0, 35)
        PasteWithOffset(0, -35)
    endif
    OverlapIntersect()
    Select(65575); Copy() # Temporary glyph
    Select(${address_store_ssliga}) # 保管所
    if (input_list[i] == "${input_latin_regular}")
        PasteWithOffset(-62, 0)
    else
        PasteWithOffset(-96, 0)
    endif
    OverlapIntersect()
...
# > (ss18 用のグリフを作る)
    ...
    Select(65575); Paste() # Temporary glyph
    if (input_list[i] == "${input_latin_regular}")
        Scale(165, 555, 384 + ${move_y_math})
    else
        Scale(155, 597, 384 + ${move_y_math})
    endif
    Copy()
...
# Change the scale of hankaku glyphs
 ...
 #        SelectMore(0u1d538, 0u1d56b) # 数学用英数字記号
        ...
        SelectMore(${address_store_zero}, ${address_store_zero} + 5) # 保管したドット無し0
        SelectMore(${address_store_zero} + 6, ${address_store_zero} + 5 + 6) # 保管したスラッシュ0
...
# 記号を一部クリア
    ...
    if (input_list[i] == "${input_latin_regular}")
        ...
        if ("${liga_flag}" == "true")
            SelectMore(65571, 65587)
            SelectMore(65591, 65672)
        else
            Select(65536, 65699)
        endif
    endif
    Clear(); DetachAndRemoveGlyphs() # 異体字等
...
# グリフ名を修正
    ...
    SetGlyphName("lessequalgreater2") # そのままのグリフ名だと合成時に消去される
...
# 半角英数記号を全角形にコピー、加工
 ...
 #    SetWidth(${width_zenkaku})
    ...
    Select(0u203c); Paste() # ‼
    Move(76, 0)
    Select(0u203c); PasteWithOffset(438, 0) # ‼
    ...
    Select(0u2047); Paste() # ⁇
    Move(60, 0)
    Select(0u2047); PasteWithOffset(454, 0) # ⁇
    Move(-${move_x_hankaku}, 0)
    SetWidth(${width_zenkaku})
    ...
    Move(76, 0)
    Select(0u0021); Copy() # !
    Select(0u2048); PasteWithOffset(438, 0) # ⁈
    Move(-${move_x_hankaku}, 0)
    SetWidth(${width_zenkaku})
    ...
    Move(76, 0)
    Select(0u003F); Copy() # ?
    Select(0u2049); PasteWithOffset(438, 0) # ⁉
    Move(-${move_x_hankaku}, 0)
    SetWidth(${width_zenkaku})
...
# ss 対応 (lookup の数を変えた場合は table_modificator も変更すること)
    ...
    while (0 < j) # ssルックアップの数だけ確保する
        if (j == ${num_ss_lookups})
            lookupName = "'ss" + ToString(19) + "' スタイルセット" + ToString(19)
            AddLookup(lookupName, "gsub_single", 0, [["ss" + ToString(19),[["DFLT",["dflt"]]]]], lookups[numlookups - 1])
        elseif (j < 10)
...
# ss10 ドット無し0
    ...
    # 小数
    Select(${address_store_zero}); Copy() # ドット無し0
    Select(k); Paste()
    Scale(${scale_calt_decimal}, ${scale_calt_decimal}, ${width_hankaku} / 2, 0)
    SetWidth(${width_hankaku})
    glyphName = GlyphInfo("Name")
...
# ss19 スラッシュ0
    ...
    # 小数
    Select(${address_store_zero} + 6); Copy() # スラッシュ0
    Select(k); Paste()
    Scale(${scale_calt_decimal}, ${scale_calt_decimal}, ${width_hankaku} / 2, 0)
    SetWidth(${width_hankaku})
    glyphName = GlyphInfo("Name")
...
# 一旦削除した sups を追加
 ...
 #    endloop
    ...
    sups = [0u1d44, 0u1d45, 0u1d9b, 0u1d53,\
            ...
            0u1dab, 0u02b1, 0u02b4, 0u02b5,\
            0u02b6, 0u02c0, 0u02c1, 0u02e0]
            # ᵄᵅᶛᵓ ᶝᶡ ᶢᶣᶤᶥ ᶦᶩᵚᶭ ᶬᶮᶯᶰ ᶱᶲᶳᶴ ᶶᶷᶹᶺ ᶼᶽᶾᶨ ᶫʱʴʵ ʶˀˁˠ
    j = 0
    while (j < SizeOf(orig))
        Select(sups[j])
        Move(0, ${move_y_super_base}) # latin フォントに高さを合わせる
...
# 半角・全角形、縦書き用を作り直し
    ...
    Select(${address_store_zero} + 3); Copy() # 保管した全角のドット無し0
    Select(${address_store_zero} + 4); Paste() # 保管した横書きのドット無し0
    Select(${address_store_underline}); Copy() # 下線追加
    Select(${address_store_zero} + 4); PasteInto() # 保管した横書きのドット無し0
    SetWidth(${width_zenkaku})

    Select(${address_store_zero} + 3); Copy() # 保管した全角のドット無し0
    Select(${address_store_zero} + 5); Paste() # 保管した縦書きのドット無し0
    Select(${address_store_underline} + 2); Copy() # 縦線追加
    Select(${address_store_zero} + 5); PasteInto() # 保管した縦書きのドット無し0
    SetWidth(${width_zenkaku})

    Select(${address_store_zero} + 3 + 6); Copy() # 保管した全角のスラッシュ0
    Select(${address_store_zero} + 4 + 6); Paste() # 保管した横書きのスラッシュ0
    Select(${address_store_underline}); Copy() # 下線追加
    Select(${address_store_zero} + 4 + 6); PasteInto() # 保管した横書きのスラッシュ0
    SetWidth(${width_zenkaku})

    Select(${address_store_zero} + 3 + 6); Copy() # 保管した全角のスラッシュ0
    Select(${address_store_zero} + 5 + 6); Paste() # 保管した縦書きのスラッシュ0
    Select(${address_store_underline} + 2); Copy() # 縦線追加
    Select(${address_store_zero} + 5 + 6); PasteInto() # 保管した縦書きのスラッシュ0
    SetWidth(${width_zenkaku})
    ...
    Select(${address_store_zero} + 3); Copy() # 保管した全角のドット無し0
    Select("uniFF10.ss10"); Paste() # 横書きのドット無し0
    Select(${address_store_underline}); Copy() # 下線追加
    Select("uniFF10.ss10"); PasteInto() # 横書きのドット無し0
    SetWidth(${width_zenkaku})

    Select(${address_store_zero} + 3); Copy() # 保管した全角のドット無し0
    Select("uniFF10.vert.ss10"); Paste() # 縦書きのドット無し0
    Select(${address_store_underline} + 2); Copy() # 縦線追加
    Select("uniFF10.vert.ss10"); PasteInto() # 縦書きのドット無し0
    SetWidth(${width_zenkaku})

    Select(${address_store_zero} + 3 + 6); Copy() # 保管した全角のスラッシュ0
    Select("uniFF10.ss19"); Paste() # 横書きのスラッシュ0
    Select(${address_store_underline}); Copy() # 下線追加
    Select("uniFF10.ss19"); PasteInto() # 横書きのスラッシュ0
    SetWidth(${width_zenkaku})

    Select(${address_store_zero} + 3 + 6); Copy() # 保管した全角のスラッシュ0
    Select("uniFF10.vert.ss19"); Paste() # 縦書きのスラッシュ0
    Select(${address_store_underline} + 2); Copy() # 縦線追加
    Select("uniFF10.vert.ss19"); PasteInto() # 縦書きのスラッシュ0
    SetWidth(${width_zenkaku})
...
# ドット無し0
    if ("${zero_mode}" == "0")
        Print("Option: Disable doted zero")
        # 半角、全角
